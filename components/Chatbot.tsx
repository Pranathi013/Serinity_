...
const Chatbot: React.FC<ChatbotProps> = ({ initialMessage, onTriggerEmergency, achievements }) => {
  ...
  useEffect(() => {
    ...
    if (initialMessage) {
      const userMessage: Message = { author: MessageAuthor.USER, text: initialMessage };
      setMessages([userMessage]);
      setIsLoading(true);

      sendMessageToChat(chatRef.current, initialMessage).then(botResponse => {
        ...
        const botMessage: Message = { author: MessageAuthor.BOT, text: finalBotResponse };
        setMessages((prev) => [...prev, botMessage]);
        setIsLoading(false);
      }).catch(() => {
        const errorMessage: Message = { author: MessageAuthor.BOT, text: "Sorry, I'm having trouble responding right now." };
  ...
  const handleSend = useCallback(async () => {
    ...
    try {
      ...
      const botMessage: Message = { author: MessageAuthor.BOT, text: botResponse };
      setMessages((prev) => [...prev, botMessage]);
    } catch (error) {
      const errorMessage: Message = { author: MessageAuthor.BOT, text: "Sorry, I'm having trouble responding right now." };
      setMessages((prev) => [...prev, errorMessage]);
